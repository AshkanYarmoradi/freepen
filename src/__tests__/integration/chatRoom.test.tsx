...
import { sendMessage, useRoomMessages } from '@/lib/db';
...
jest.mock('@/lib/db', () => ({
  ...
  useRoomMessages: jest.fn().mockImplementation((roomId, callback) => {
...
describe('Chat Room', () => {
  ...
  it('should display messages from the room', async () => {
    ...
    // Check that useRoomMessages was called with the correct room ID
    expect(useRoomMessages).toHaveBeenCalledWith('test-room-id', expect.any(Function));
  ...
  it('should display a message when there are no messages', () => {
    // Override the useRoomMessages mock for this test
    (useRoomMessages as jest.Mock).mockImplementationOnce((roomId, callback) => {
      callback([]);
      return jest.fn();
    });
